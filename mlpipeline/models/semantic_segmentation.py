...
from mlpipeline.models.segmentation.transbts.TransBTS_downsample8x_skipconnection import BTS
from mlpipeline.models.segmentation.nestedformer.nested_former import NestedFormer
...
class SemanticSegmentation(nn.Module):
    ...
    def get_model(self):
        ...
        elif self.cfg.arch.lower() == 'TransBTS'.lower():
            self.model = BTS(img_dim=self.cfg.image_size,
                            img_depth=self.cfg.image_depth,
                            patch_dim=8,
        ...
        elif self.cfg.arch.lower() == 'NestedFormer'.lower():
            self.model = NestedFormer(model_num=self.cfg.num_channels,
                                    out_channels=self.cfg.num_classes,
                                    image_size=(self.cfg.image_size, self.cfg.image_size, self.cfg.image_depth),
                                    window_size=(self.cfg.image_size // 32, self.cfg.image_size // 32, self.cfg.image_depth // 32)
            )
        else:
